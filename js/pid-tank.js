...
class PIDTankSimulator {
    ...
    stepSimulation() {
        ...
        const deriv = -(this.h - this.prevH) / this.dt;
        let u = this.params.kp * e + this.params.ki * this.integral + this.params.kd * deriv;
        this.prevError = e;
        this.prevH = this.h;

        // Clamp control
        u = Math.max(0, Math.min(20, u));
    ...
    initPlots() {
        ...
        this.currentChart = 'tank';
    }

    switchChart(chart) {
        this.currentChart = chart;
        this.chartTabs.forEach(t => t.classList.toggle('active', t.dataset.chart === chart));
        document.getElementById('chart-tank').style.display = chart === 'tank' ? 'block' : 'none';
        document.getElementById('chart-control').style.display = chart === 'control' ? 'block' : 'none';
        document.getElementById('chart-error').style.display = chart === 'error' ? 'block' : 'none';
        // Refresh data on the newly visible chart, then resize
        this.updatePlots();
        setTimeout(() => {
